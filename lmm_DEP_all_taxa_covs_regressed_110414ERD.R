...
###### 
# This script will do a lmm for the DEP samples, with rank as a fixed effect and animal, run, and phase as random effects. Mastermix and date collected are regressed out first. 
# usage: ./lmm_DEP_all_taxa_covs_regressed_110414ERD.R
...
write.table(assocs, paste0("../results/7_lmm_covs/table_p_and_q_values_lmm_",today,"ERD.txt"), sep="\t", row.names=FALSE, quote=FALSE)
...
write.table(qval.summary, paste0("../results/7_lmm_covs/table.summary.lmm.q.values.at.various.cutoffs.",today,"ERD.txt", sep=""), sep="\t", quote=FALSE)



##### Histogram and qqplot:
pdf(paste0("../results/7_lmm_covs/hist_qqplot_lmm_pvalues_",today,"ERD.pdf"), width=10, height=5)
par(mfrow=c(1,2))
