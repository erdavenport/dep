...
# usage: ./lmm_phase_DEP_all_taxa_covs_regressed_no_col_date_111214ERD.R
...
for (a in c("phylum", "class", "order", "family", "genus")) {
	...
	for (i in 1:dim(taxa)[1]) {
		...
		mylm <- lm(as.numeric(taxa[i,]) ~ as.factor(covs$Mastermix), na.action=na.exclude)
		resids <- resid(mylm)	
...
write.table(assocs, paste0("../results/16_lmm_of_phase/table_p_and_q_values_lmm_no_col_date",today,"ERD.txt"), sep="\t", row.names=FALSE, quote=FALSE)
...
write.table(qval.summary, paste0("../results/16_lmm_of_phase/table.summary.lmm.q.values.at.various.cutoffs.no.col.date.",today,"ERD.txt", sep=""), sep="\t", quote=FALSE)



##### Histogram and qqplot:
pdf(paste0("../results/16_lmm_of_phase/hist_qqplot_lmm_pvalues_no_col_date_",today,"ERD.pdf"), width=10, height=5)
par(mfrow=c(1,2))
