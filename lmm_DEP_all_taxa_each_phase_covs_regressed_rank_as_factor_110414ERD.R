...
###### 
# This script will do a lmm for the DEP samples, with rank as a fixed effect and run as a random effect. This is for each phase individually. Mastermix and date collected are regressed out first. Rank as factor in this iteration.
# usage: ./lmm_DEP_all_taxa_each_phase_covs_regressed_rank_as_factor_110414ERD.R
...
for (p in c("phase_1", "phase_2", "phase_3", "phase_4")) {
	...
	for (a in c("phylum", "class", "order", "family", "genus")) {
		...
		for (i in 1:dim(taxa)[1]) {
			...
			mytry2 <- try(mymodel <- lmer(as.numeric(resids) ~ as.factor(covs$Rank) + (1|as.factor(covs$Run)), REML=FALSE), silent=TRUE)
	...
	write.table(assocs, paste0("../results/10_lmm_phase_covs_factor/table_p_and_q_values_lmm_",p,"_",today,"ERD.txt"), sep="\t", row.names=FALSE, quote=FALSE)
	...
	write.table(qval.summary, paste0("../results/10_lmm_phase_covs_factor/table.summary.",p,".lmm.q.values.at.various.cutoffs.",today,"ERD.txt", sep=""), sep="\t", quote=FALSE)



	##### Histogram and qqplot:
	pdf(paste0("../results/10_lmm_phase_covs_factor/hist_qqplot_lmm_pvalues_",p,"_",today,"ERD.pdf"), width=10, height=5)
	par(mfrow=c(1,2))
...
write.table(qvaltab, paste0("../results/10_lmm_phase_covs_factor/table.summary.all.phases.lmm.q.values.at.various.cutoffs.",today,"ERD.txt", sep=""), sep="\t", quote=FALSE)
