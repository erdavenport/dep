...
##### Divide lines by 4:
lines <- read.table(file=paste(input.path, "FC3_fastq_lines.txt", sep=""), sep="\n", header=FALSE)$V1

liners <- matrix(lines, ncol=2, byrow=TRUE)

liners[,1] <- gsub(".txt.gz", "", liners[,1])
liners[,2] <- as.numeric(liners[,2])/4
write.table(liners, paste("../results/1_general/reads_sequenced_per_sample_FC3_with_Hutterites_",today,"ERD.txt", sep=""), sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
...
pdf(paste("../results/1_general/plot.sequencing.by.phase.w.Hutterites.FC3.",today,"ERD.pdf", sep=""))
...
abline(h=median(as.numeric(sliners[grep("HUTT", sliners[,1]),2]))*8, col=sapply("HUTT", phase_cols))
legend("topleft", legend=c("phase 1", "phase 2", "phase 3", "phase 4", "hutterite"), fill=sapply(c("phase1", "phase2", "phase3", "phase4", "HUTT"), phase_cols))



##### Only look at DEP samples:
liners2 <- liners[-grep("HUTT", liners[,1]),]
write.table(liners, paste("../results/1_general/reads_sequenced_per_sample_FC3_",today,"ERD.txt", sep=""), sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)

sliners <- liners2[order(as.numeric(liners2[,2])),]


pdf(paste("../results/1_general/plot.sequencing.by.phase.FC3.",today,"ERD.pdf", sep=""))
barplot(as.numeric(sliners[,2]), col=sapply(sliners[,1], phase_cols), main="DEP sequencing by phase", xlab="sample", ylab="reads")
abline(h=median(as.numeric(sliners[grep("phase1", sliners[,1]),2])), col=sapply("phase1", phase_cols))
abline(h=median(as.numeric(sliners[grep("phase2", sliners[,1]),2])), col=sapply("phase2", phase_cols))
abline(h=median(as.numeric(sliners[grep("phase3", sliners[,1]),2])), col=sapply("phase3", phase_cols))
