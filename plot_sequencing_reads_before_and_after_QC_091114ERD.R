...
###### 
# This script will generate a plot of the reads before and after read QC for FC3.
...
qced <- read.table(file=paste(input.path, "lines_of_qced_fastq_file_091114ERD.txt", sep=""), sep=" ", header=FALSE)
...
pdf(paste("../results/1_general/plot.sequencing.after.read.QC.FC3.with.hutts.",today,"ERD.pdf", sep=""), width=10, height=5)
par(mfrow=c(1,2))
...
##### Plot only the DEP samples
s.merged <- s.merged[-grep("HUTT", s.merged$sample),]
pdf(paste("../results/1_general/plot.sequencing.after.read.QC.FC3.",today,"ERD.pdf", sep=""), width=10, height=5)
par(mfrow=c(1,2))
barplot(as.numeric(as.character(s.merged$sequenced)), col="gray80", main="DEP: reads sequenced/after QC", xlab="sample", ylab="reads")
barplot(as.numeric(as.character(s.merged$qced)), col="gray50", add=TRUE)
legend("topleft", fill=c("gray80", "gray50"), legend=c("reads sequenced", "reads after QC"))
