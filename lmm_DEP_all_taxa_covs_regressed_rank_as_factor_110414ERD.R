...
###### 
# This script will do a lmm for the DEP samples, with rank as a fixed effect and animal, run, and phase as random effects. Mastermix and date collected are regressed out first. In this iteration, rank is a factor.
# usage: ./lmm_DEP_all_taxa_covs_regressed_rank_as_factor_110414ERD.R
...
for (a in c("phylum", "class", "order", "family", "genus")) {
	...
	for (i in 1:dim(taxa)[1]) {
		...
		mytry2 <- try(mymodel <- lmer(as.numeric(resids) ~ as.factor(covs$Rank) + (1|as.factor(covs$Animal)) + (1|as.factor(covs$Phase)) + (1|as.factor(covs$Run)), REML=FALSE), silent=TRUE)
...
write.table(assocs, paste0("../results/9_lmm_covs_factor/table_p_and_q_values_lmm_",today,"ERD.txt"), sep="\t", row.names=FALSE, quote=FALSE)
...
write.table(qval.summary, paste0("../results/9_lmm_covs_factor/table.summary.lmm.q.values.at.various.cutoffs.",today,"ERD.txt", sep=""), sep="\t", quote=FALSE)



##### Histogram and qqplot:
pdf(paste0("../results/9_lmm_covs_factor/hist_qqplot_lmm_pvalues_",today,"ERD.pdf"), width=10, height=5)
par(mfrow=c(1,2))
