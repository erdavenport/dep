...
###### 
# This script will plot associations for the LMM for phase, mastermix regressed out
# usage: ./plot_lmm_phase_covs_regressed_significant_associations_111214ERD.R
...
covs1$Phase <- droplevels(covs1$Phase)


##### Read in qvalue table:
qvals <- read.table(file="../results/16_lmm_of_phase/table_p_and_q_values_lmm_no_col_date111214ERD.txt", sep="\t", header=TRUE)
...
for (i in 1:dim(qvals)[1]) {
	...
	pdf(paste0("../results/16_lmm_of_phase/boxplot_",qvals$bacteria[i],"_by_phase_",today,"ERD.pdf"))
	stripchart(as.numeric(alltaxa[myi,]) ~ covs1$Phase, vertical=TRUE, xlab="phase", method="jitter", ylab="abundance", pch=16, col="gray", main=paste0(qvals$bacteria[i]," by phase\np-value = ",round(as.numeric(as.character(qvals$pvalue[i])), 4)," q-value = ",round(as.numeric(as.character(qvals$qvalue[i])), 4)))
	suppressWarnings(boxplot(as.numeric(alltaxa[myi,]) ~ covs1$Phase, add=TRUE, boxwex=0.35, names=rep(" ", 4), notch=TRUE, outline=FALSE)) 
	dev.off()
}
